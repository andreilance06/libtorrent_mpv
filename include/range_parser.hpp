...
/**
 * Generic range exception. Usually thrown when an invalid range is being
 * parsed.
 */
...
struct Range {
  ...
  /**
   * Creates a content range string, suitable to be used in "Content-Range"
   * header.
   *
   ...
   */
  std::string content_range(std::int64_t pSize,
                            const char *pUnit = UNIT_BYTES) const;
};

/**
 * Struct defining a generic HTTP range.
 * This can be either a single part or a multipart range. Such can be determined
 * by checking the length of \ref HTTPRange.ranges.
 */
struct HTTPRange {
  /// The HTTP range unit.
  std::string unit;
